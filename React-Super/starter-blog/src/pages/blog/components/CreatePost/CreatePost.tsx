import { useAddPostMutation, useGetPostQuery, useUpdatePostMutation } from 'pages/blog/redux/post.service'
import { postCancelAction } from 'pages/blog/redux/post.slice'
...
export default function CreatePost() {
  const [formData, setFormData] = useState<Omit<PostType, 'id'> | PostType>(initFormData)
  ...
  console.log(data, '12')
  const [postAdd, postResult] = useAddPostMutation()
  const [postUpdate, postUpdateResult] = useUpdatePostMutation()
  const handleSubmitPost = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    try {
      if (postId) {
        await postUpdate({ id: postId, body: formData as PostType }).unwrap()
        setFormData(initFormData)
        disphatch(postCancelAction())
      } else {
        await postAdd(formData).unwrap()
      }
    } catch (error) {
      console.log(error)
    }
